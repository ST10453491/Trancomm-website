document.addEventListener('DOMContentLoaded', () => {
  ...
  const detailFrame = document.querySelector('.contracts-detail-section iframe');
  ...
  document.querySelectorAll('[data-portal-target]').forEach((link) => {
    link.addEventListener('click', (event) => {
      ...
      const target = document.getElementById(link.dataset.portalTarget);
      if (!target) return;
      document.querySelectorAll('[data-portal-target]').forEach((item) => item.classList.remove('active'));
      document.querySelectorAll('.employee-portal-panel').forEach((panel) => panel.classList.remove('active'));
